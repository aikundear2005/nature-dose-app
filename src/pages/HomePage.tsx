...
const HomePage = () => {
  ...
  // --- 主要修改處：強化 fetch 的錯誤處理 ---
  const handleApiResponse = async (response: Response) => {
    if (response.ok) {
      // 只有在回應成功時才解析 JSON
      // 如果回應是 204 No Content，也會回傳 null
      return response.status === 204 ? null : response.json();
    }
    // 如果回應不成功 (4xx, 5xx)，嘗試解析錯誤訊息
    const errorBody = await response.text();
    try {
      const errorJson = JSON.parse(errorBody);
      throw new Error(errorJson.message || errorJson.error || `伺服器錯誤: ${response.status}`);
    } catch {
      throw new Error(errorBody || `伺服器錯誤: ${response.status}`);
    }
  };
  ...
  const fetchNearbyPlaces = useCallback(async (lat: number, lon: number) => {
    ...
    try {
      ...
      const transformedPlaces: Place[] = data.features.map((item: any) => ({
        id: item.id,
  ...
  const getNatureDataFromLocation = useCallback(async (lat: number, lon: number) => {
    ...
    try {
      ...
      let score = 2;
      let env = '街道社區';
      const mainFeatureType = data.features[0]?.place_type[0];
      ...
      setLocationError(apiError.message || '無法從伺服器獲取地點資訊。');
    }
  }, []);
  ...
  }, [getNatureDataFromLocation, fetchNearbyPlaces]);
    ...
    <div className="max-w-md mx-auto bg-gray-900 text-white min-h-screen font-sans">
      ...
      {isModalOpen && (
        <div
          ...
          aria-modal="true"
        >
          <div className="bg-gray-800 border border-gray-700 rounded-2xl shadow-xl p-6 w-full max-w-sm text-white">
            <div className="flex justify-between items-center mb-4">
      ...
      {isChangelogOpen && (
        <div
          ...
          aria-modal="true"
        >
          <div className="bg-gray-800 border border-gray-700 rounded-2xl shadow-xl p-6 w-full max-w-md text-white">
            <div className="flex justify-between items-center mb-4">
