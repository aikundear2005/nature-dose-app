...
import { type Place } from '../types'; // <-- 新增此行，從新檔案匯入 Place 類型
export type Place = {
  id: number | string;
  name: string;
  distance: number;
...
const PlaceCard = ({ place, isExpanded, onToggle }: PlaceCardProps) => {
  ...
  // 修正：Google Maps Search URL 應指向 maps.google.com
  const mapSearchUrl = `https://maps.google.com/?q=${encodeURIComponent(
    place.name
  )}`;
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        className="p-4 w-full text-left transition-colors hover:bg-white/5"
        onClick={onToggle}
        aria-expanded={isExpanded} // 增加aria屬性，提升無障礙性
      >
      ...
      {isExpanded && (
        <div className="px-4 pb-4 border-t-2 border-dashed border-gray-700 mt-2 pt-3 animate-fade-in">
          <div className="text-sm text-gray-300 space-y-2 mb-4">
            <p>
              <span className="font-semibold text-gray-100">特色：</span>
              {place.description}
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3 text-sm">
            <a
              ...
              className="flex items-center justify-center space-x-2 bg-black bg-opacity-20 py-2 rounded-lg font-semibold text-gray-200 hover:bg-opacity-30 transition-colors"
            >
            ...
            <a
              href={navigationUrl}
              className="flex items-center justify-center space-x-2 bg-blue-600 text-white py-2 rounded-lg font-semibold hover:bg-blue-700 transition-colors"
            >
